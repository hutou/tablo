  ...
  context "Initialization with different types of Enumerable" do
    context "#initialize with block given" do
      context "from an Array(Int32)" do
        it "creates and displays a new table from Array(Int32)" do
      ...
      context "from a hash(String, Int32)" do
        it "creates and displays a new table from Hash(String, Int32)" do
      ...
      context "from a Range(Int32)" do
        it "creates a new table from Range(Int32..Int32)" do
          ...
          table.should be_a(Tablo::Table(Int32))
          #
          # Up to and including Crystal 1.15, Range is not supported in the Tablo context.
          # (An invalid memory access error is generated by the compiler)
          #
        end
      end
      context "from an array of Struct" do
        it "creates and displays a new table from Array(Person)" do
      ...
      context "from a user defined enumerable class" do
        it "creates and displays a new table from OddSamples enumerable class" do
        ...
        it "creates and displays a new table from a populated SQL database" do
          ...
          1.upto 9 do |num|
            conn_db.exec "INSERT INTO game (name, score) VALUES(?, ?)",
              "G#{num}", num.odd? ? num * 3 : num * 2
          end
          sql = "SELECT name, score FROM game;"
          table = Tablo::Table.new(conn_db.query_all(sql, as: {String, Int32})) do |t|
            t.add_column("game") { |n| n[0] }
            t.add_column("score") { |n| n[1] }
          end
          conn_db.close
          table.should be_a(Tablo::Table({String, Int32}))
          output = table.to_s
          {% if flag?(:DEBUG) %} puts "\n#{output}" {% end %}
          expected_output = <<-OUTPUT
            +--------------+--------------+
    ...
    context "#initialize *without* block given" do
      ...
      context "from a SQL query" do
        it "creates and displays a new table from a populated SQL database" do
          ...
          sql = "SELECT name, score FROM game;"
          table = Tablo::Table.new(conn_db.query_all(sql, as: {String, Int32}))
          table.add_column("game") { |n| n[0] }
          table.add_column("score") { |n| n[1] }
          conn_db.close
          table.should be_a(Tablo::Table({String, Int32}))
          output = table.to_s
          {% if flag?(:DEBUG) %} puts "\n#{output}" {% end %}
