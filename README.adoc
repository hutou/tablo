...
The Cell `value:` attribute contains the raw data, either extracted from the source
for Body rows (DataCell) or directly supplied as an argument for other row
types (TextCell).  
...
- The second expects an additional argument (of type Int32) corresponding to
  the column width (special use cases, like text stretching)
