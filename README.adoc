...
| `title`, `subtitle`, `footer` | `UnFramedHeading` \|  `FramedHeading` =
`FramedHeading.new` | Initializing an (`Un`)`FrameHeading` class without any argument sets
its value to `nil`, so there is nothing to display 
...
| `title_repeated` | `Bool` = `false` | If `true`, force title and subtitle to
be repeated at header frequency 

| `footer_page_break` | `Bool` = `false` |  If `true`, emit a page break
after footer
|===
...
|`label` |  `LabelType` | The label identifies the column. It can be an `Int32`,
...
|`label` |  `LabelType` | The label identifies the group. It can be an `Int32`,
a `Symbol` or a `String`
...
| `width`   |   `TableWidth? = GetWidthFrom::Screen` |
| `starting_widths`   |   `StartingWidths = Config.starting_widths` |
| `except`   |   `Except? = nil` |
|===

the pack method falls down  to its previous state in the event of failure
...
- `width`: the total width of the formatted table. With positive `width`, this
  is a maximum value, and with negative `width`, a minimum value. If the value
  of parameter `Config.terminal_capped_width` is true, the value of
  `width`, if not directly supplied, is the number of columns in
  the terminal. 

- `autosize` : if true (default), column widths are automatically adapted to their
  largest content, before any attempt is made to increase or reduce the total
  width of the table. 

- `except`: column label (or label array) to be excluded from resizing
  processing.

If, with the parameter `autosize = true` by default (set by
`Config.pack_autosize`), the `pack` method automatically adapts the width of
columns to their largest content (body or header) before resizing, this
requires you to go through the entire source dataset, which can be costly in
terms of performance in some cases.  

This behavior can be avoided with the `autosize = false` parameter: in this
case, however, a positive or negative value for `width` must be supplied,
either directly or by reading the terminal width, otherwise `pack` will become
a non-operation.  

=== classes FramedHeading and UnFramedHeading
