...
[false, true].each do |detached|
  [false, true].each do |bordered|
    # [false, true].each do |page_break|
    [3].each do |header_frequency|
      table = Tablo::Table.new((1..7).to_a,
        header_frequency: header_frequency,
        row_divider_frequency: nil,
        title: Tablo::Title.new("\nDetail\n", frame: Tablo::Frame.new, repeated: true),
        # masked_headers: true,
        # subtitle: Tablo::Heading.new("Detail subtitle",
        #   bordered: false),
        # subtitle: Tablo::Heading.new(" header_frequency: #{header_frequency}"),

        footer: Tablo::Footer.new("My footer\ndetached: #{true}, bordered: #{true}", frame: Tablo::Frame.new),
        subtitle: Tablo::SubTitle.new(
          "My subtitle\n" +
          "detached: #{detached}, bordered: #{bordered}, " +
          " header_frequency: #{header_frequency}", frame: Tablo::Frame.new),
        # detached: detached,
        # bordered: bordered),
