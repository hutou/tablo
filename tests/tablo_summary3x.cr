...
table = Tablo::Table.new([1, "abc", 2, 3, 4, "つのだ☆HIRO", 5, 37],
  ...
  omit_last_rule: true,
  title: Tablo::Title.new("Detail", frame: Tablo::Frame.new(2, 0), repeated: true),
  subtitle: Tablo::SubTitle.new("Detail subtitle"),
  footer: Tablo::Footer.new("Ah ah", frame: Tablo::Frame.new),
  #   bordered: false, detached: false, page_break: false),
  # border_type: "╭┬╮├┼┤╰┴╯│:│TGHBM",
  ...
  t.summary({
    "A" => {
      ...
      body_2: ->(ary : Tablo::NumCol) { ar = ary.compact; ar.size > 0 ? (ar.sum / ar.size) : nil },
      # body_2:         ->(ary : Tablo::NumCol) { ary.sum / ary.size },
    ...
    "C" => {
      ...
      # proc2:          ->(ary : Tablo::NumCol) { ary.size > 0 ? (ary.sum/ary.size).to_s : "NA" },
    },
    "D" => {
      body_alignment: Tablo::Justify::Left,
    ...
    # title: Tablo::Heading.new("Summary",
